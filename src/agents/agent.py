...
class Agent:
    ...
    def make_decision(self, prompt: str, project_name: str) -> str:
        ...
        for item in decision:
            ...
            ProjectManager().add_message_from_StartifyAI(project_name, reply)
            ...
            if function == "git_clone":
                ...
                ProjectManager().add_message_from_StartifyAI(project_name, response)
    ...
    def execute(self, prompt: str, project_name_from_user: str = None) -> str:
        ...
        ProjectManager().add_message_from_StartifyAI(project_name, reply)
        ProjectManager().add_message_from_StartifyAI(project_name, json.dumps(plans, indent=4))
        # ProjectManager().add_message_from_StartifyAI(project_name, f"In summary: {summary}")
        ...
        ProjectManager().add_message_from_StartifyAI(
            project_name,
        ...
        if ask_user != "":
            ProjectManager().add_message_from_StartifyAI(project_name, ask_user)
            AgentState().set_agent_active(project_name, False)
            ...
            while not got_user_query:
                ...
                if latest_message_from_user and validate_last_message_is_from_user:
                    ...
                    got_user_query = True
                    ProjectManager().add_message_from_StartifyAI(project_name, "Thanks! 🙌")
                time.sleep(5)
