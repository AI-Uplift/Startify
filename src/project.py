...
class ProjectManager:
    ...
    def add_message_from_StartifyAI(self, project: str, message: str):
        ...
        with Session(self.engine) as session:
            ...
            if project_state:
                ...
                for message in reversed(message_stack):
                    if not message["from_StartifyAI"]:
                        return message
            return None
    ...
    def validate_last_message_is_from_user(self, project: str):
        with Session(self.engine) as session:
            ...
            if project_state:
                message_stack = json.loads(project_state.message_stack_json)
                if message_stack:
                    return not message_stack[-1]["from_StartifyAI"]
            return False

    def get_latest_message_from_StartifyAI(self, project: str):
        with Session(self.engine) as session:
            project_state = session.query(Projects).filter(Projects.project == project).first()
            if project_state:
                message_stack = json.loads(project_state.message_stack_json)
                for message in reversed(message_stack):
                    if message["from_StartifyAI"]:
                        return message
            return None
        ...
        with Session(self.engine) as session:
            ...
            if project_state:
                ...
                for message in message_stack:
                    if message["from_StartifyAI"]:
                        formatted_messages.append(f"StartifyAI: {message['message']}")
                    else:
